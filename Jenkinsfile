	...
	stage('Submit Stack') {
		...
		//    }
		    ...
		    // {
				    ...
				    Start-Process powershell -verb runas -ArgumentList "echo $env.WORKSPACE"
				    Start-Process powershell -verb runas -ArgumentList "aws cloudformation create-stack --stack-name s3bucket --template-body file://$env.WORKSPACE/stack.json --region 'us-east-1'"
		     sh "sudo -su"	    
